...
XBB_APPLICATION_NAME=${XBB_APPLICATION_NAME:-"QEMU RISC-V"}

# Used as part of file/folder paths (keep uppercase).
XBB_APPLICATION_LOWER_CASE_NAME=${XBB_APPLICATION_LOWER_CASE_NAME:-"qemu-riscv"}
...
declare -a XBB_APPLICATION_DEPENDENCIES=( qemu-riscv )
declare -a XBB_APPLICATION_COMMON_DEPENDENCIES=( libiconv autoconf automake libtool pkg-config m4 zlib bzip2 zstd gmp libpng jpeg libxml2 sdl2 libffi gettext termcap readline pcre2 glib openssl libssh ncurses libusb-w32 libusb1 lzo nettle pixman libpcap vde qemu )
