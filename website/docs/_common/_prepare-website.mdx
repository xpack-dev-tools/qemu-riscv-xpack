...
[GitHub Pages](https://xpack-dev-tools.github.io/qemu-riscv-xpack/).

- Switch to the `xpack-development` branch
- Execute the **website-generate-commons** npm script in the `website/package.json`
  ```sh
  npm --prefix ~/Work/xpack-dev-tools/qemu-riscv-xpack.git/website run website-generate-commons
  ```
- **Commit** all changes
