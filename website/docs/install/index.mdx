...
The easiest (and recommended) way to install xPack QEMU RISC-V
...
* add <code>@xpack-dev-tools/qemu-riscv</code> to
  `package.json` as a
  development dependency; this associates a specific version of xPack QEMU RISC-V with
...
it is also possible to install xPack QEMU RISC-V only globally in
...
In order to access the xPack QEMU RISC-V binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/qemu-riscv/8.2.6-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/qemu-riscv/8.2.6-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<ul>
<li><code>xpack-qemu-riscv-8.2.6-1-win32-x64.zip</code></li>
</ul>
...
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\qemu-riscv\\xpack-qemu-riscv-8.2.6-1</code>.
...
<ul>
<li><code>xpack-qemu-riscv-8.2.6-1-darwin-x64.tar.gz</code></li>
<li><code>xpack-qemu-riscv-8.2.6-1-darwin-arm64.tar.gz</code></li>
</ul>
...
example <code>/Users/ilg/Library/xPacks/qemu-riscv/xpack-qemu-riscv-8.2.6-1</code>:
...
example <code>/home/ilg/.local/xPacks/qemu-riscv/xpack-qemu-riscv-8.2.6-1</code>:
