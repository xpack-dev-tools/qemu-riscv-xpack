...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fqemu-riscv?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/qemu-riscv/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/qemu-riscv-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fqemu-riscv?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/qemu-riscv/)

The **xPack QEMU RISC-V** can be installed automatically, using the `xpm` command
...
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/qemu-riscv`}><code>@xpack-dev-tools/qemu-riscv</code></a> from
...
<code>xpacks/@xpack-dev-tools/qemu-riscv</code>.
...
**qemu-riscv** package into the project:
...
* identify the platform specific archive for the latest available version
  of QEMU RISC-V, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/qemu-riscv</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/qemu-riscv/.content/bin</code>;
* add <code>@xpack-dev-tools/qemu-riscv</code> to
  `package.json` as a
  development dependency; this associates a specific version of QEMU RISC-V with
...
it is also possible to install QEMU RISC-V only globally in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/qemu-riscv/${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/qemu-riscv/${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/qemu-riscv/${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
the <a href={`https://github.com/xpack-dev-tools/qemu-riscv-xpack/releases/`}>GitHub Releases</a> pages.
...
* <code>xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}-win32-x64.zip</code>
...
To manually install the <b>xPack QEMU RISC-V</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\qemu-riscv</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\qemu-riscv\\xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>.
...
* <code>xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-x64.tar.gz</code>
* <code>xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-arm64.tar.gz</code>
...
The recommended location is
the <code>~/Library/xPacks/qemu-riscv</code> folder, for
example <code>/Users/ilg/Library/xPacks/qemu-riscv/xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/qemu-riscv
cd ~/Library/xPacks/qemu-riscv

tar xvf ~/Downloads/xpack-qemu-riscv-${customField('upstreamVersion')}-${customField('xpackSubversion')}-darwin-x64.tar.gz
chmod -R -w xpack-qemu-riscv-${customField('upstreamVersion')}-${customField('xpackSubversion')}
`} </CodeBlock>
...
* <code>xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-x64.tar.gz</code>
* <code>xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-arm64.tar.gz</code>
* <code>xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-arm.tar.gz</code>
...
The recommended location is
the <code>~/.local/xPacks/qemu-riscv</code> folder, for
example <code>/home/ilg/.local/xPacks/qemu-riscv/xpack-qemu-riscv-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/qemu-riscv
cd ~/.local/xPacks/qemu-riscv

tar xvf ~/Downloads/xpack-qemu-riscv-${customField('upstreamVersion')}-${customField('xpackSubversion')}-linux-x64.tar.gz
chmod -R -w xpack-qemu-riscv-${customField('upstreamVersion')}-${customField('xpackSubversion')}
`} </CodeBlock>
