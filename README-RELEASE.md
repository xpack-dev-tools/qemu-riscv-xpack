...
Determine the version (like `6.1.0`) and update the `scripts/VERSION`
file; the format is `6.1.0-1`. The fourth number is the xPack release number
...
and fix them; assign them to a milestone (like `6.1.0-1`).
...
- add a new entry like _- v6.1.0-1 prepared_
- commit with a message like _prepare v6.1.0-1_
...
bash ~/Downloads/qemu-riscv-xpack.git/scripts/helper/build.sh --develop --linux32

bash ~/Downloads/qemu-riscv-xpack.git/scripts/helper/build.sh --develop --win64
bash ~/Downloads/qemu-riscv-xpack.git/scripts/helper/build.sh --develop --win32
```
...
- in `CHANGELOG.md`, add the release date and a message like _- v6.1.0-1 released_
- commit and push the `xpack-develop` branch
- run the xPack action `trigger-workflow-publish-release`

The result is a
[draft pre-release](https://github.com/xpack-dev-tools/qemu-riscv-xpack/releases/)
tagged like **v6.1.0-1** (mind the dash in the middle!) and
named like **xPack QEMU RISC-V v6.1.0-1** (mind the dash),
...
- commit the `develop` branch of `xpack/web-jekyll` GitHub repo;
  use a message like **xPack QEMU RISC-V v6.1.0-1 released**
- push to GitHub
...
- commit all changes, use a message like
  `package.json: update urls for 6.1.0-1.1 release` (without `v`)

## Publish on the npmjs.com server

- select the `xpack-develop` branch
- check the latest commits `npm run git-log`
- update `CHANGELOG.md`, add a line like _- v6.1.0-1.1 published on npmjs.com_
- commit with a message like _CHANGELOG: publish npm v6.1.0-1.1_
- `npm pack` and check the content of the archive, which should list
  only the `package.json`, the `README.md`, `LICENSE` and `CHANGELOG.md`;
  possibly adjust `.npmignore`
- `npm version 6.1.0-1.1`; the first 5 numbers are the same as the
...
- `npm dist-tag add @xpack-dev-tools/qemu-riscv@6.1.0-1.1 latest`
- `npm dist-tag ls @xpack-dev-tools/qemu-riscv`

In case the previous version is not functional and needs to be unpublished:

- `npm unpublish @xpack-dev-tools/qemu-riscv@6.1.0-1.X`
...
- paste the release name like **xPack QEMU RISC-V v6.1.0-1 released**
